...
def ReadBinary(binaryFile, sampleCount, featureCount):
    ...
    with open(binaryFile, "rb") as f:
        for i in np.arange(sampleCount):
            ...
            bts = f.read(bytesPerLine)
            data.append(np.float32(array.array('f', bts)))
    print('Reading binary ' + binaryFile + '...', 100, "%", end="\r")
    print("")
    return np.concatenate(data).reshape(sampleCount, -1)


def WriteBinary(binaryFile, data):
...
def LoadFullSequence(path, debug=False, lineCount=None):
    print("Loading " + path)
    data = []
    with open(path) as file:
        pivot = 0
...
def create_slices(x, window_size):
    slices = []
    for i in range(x.shape[-1] - window_size + 1):
        slices.append(x[..., i:i + window_size])
